...
cred = credentials.Certificate(r'firebase_key.json')  # Firebase 서비스 계정 키 파일 경로**ssumap것으로 변경해야함
...
try:
    ...
    while hes:
        ...
        try:
            ...
            doc_ref = db.collection("instargram").document("US:SUM")
            doc = doc_ref.get()
            description = doc.to_dict().get(f'content{index}', 'No data found')
            if description == last:
                doc_ref.update({
                    'last_content': first
                })
